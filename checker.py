...
class LinkChecker:
    ...
    def _analyze_response(self, raw_input: str, token: str, url: str, status_code: int, final_url: str, html: str, duration_ms: int) -> dict:
        ...
        if "accounts.google.com/ServiceLogin" in final_url or "accounts.google.com/v3/signin" in final_url:
            # 如果已经配置了 Cookie 但仍被重定向到登录页
            # 注意：这不代表链接无效！可能是 Cookie 已过期，或服务器端 IP 被 Google 限制
            if self.cookie_str and len(self.cookie_str.strip()) > 10:
                return {
                    ...
                    "details": "Cookie 可能已过期或被服务器拒绝（Google 仍跳转登录页，建议重新获取 Cookie 或手动打开链接确认）",
                    "plan_info": "需刷新 Cookie",
                    "expire_deadline": "-",
                    "remaining_time": "-",
                    "plan_valid_until": "-",
                    "duration_ms": duration_ms,
